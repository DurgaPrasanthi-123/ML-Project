...
# Label lengths of the multi-label suffixes, longest first — lets eTLD+1
# resolution check at most a couple of tail candidates per hostname.
_SUFFIX_LENGTHS = sorted({s.count(".") + 1 for s in MULTI_LABEL_SUFFIXES}, reverse=True)
...
# A 'host[:port]' authority followed by end-of-string, '/', '?', or '#' — the
# shape users type when omitting the scheme ('example.com:8080/admin'). The
# port part keeps the prefix from matching a bare 'schema-ish/path' string.
_BARE_AUTHORITY_RE = re.compile(
    r"^(?:[a-zA-Z0-9]([a-zA-Z0-9\-.]*[a-zA-Z0-9])?)?"  # hostname
    r"(:\d{1,5})?"                                      # optional port
    r"(?:[/?#]|$)"                                      # path/query/fragment or end
)
...
def normalize_url(raw_url: str) -> str:
    ...
    Also repairs scheme-omission shapes that would otherwise fail hardening
    checks in parse_url(), so bare input parses consistently with its explicit
    form:
    - 'example.com:8080/x'  -> 'https://example.com:8080/x'  (port, not scheme)
    - 'https:/example.com'  -> 'https://example.com'         (malformed '//')
    Non-web schemes (ftp:, mailto:, ...) are returned untouched so parse_url
    rejects them with the proper unsupported_scheme error.
    """
    if raw_url is None:
        return ""
    u = str(raw_url).strip()
    ...
    if not _has_scheme(u):
        # Scheme-relative ('//host/path') joins with 'https:'; anything else is
        # a bare authority and joins with 'https://'.
        u = ("https:" if u.startswith("//") else "https://") + u
    elif _BARE_AUTHORITY_RE.match(u):
        # 'host[:port][/path]' parses as scheme:host — add https:// so the
        # port stays a port. Restricted to authority-like prefixes so genuinely
        # invalid hostnames still fail hostname validation.
        u = "https://" + u
    else:
        scheme = u.split(":", 1)[0].lower()
        if scheme in ("http", "https"):
            rest = u.split(":", 1)[1]
            u = scheme + "://" + rest.lstrip("/")
        # else: unknown scheme left untouched; parse_url rejects unsupported_scheme
    return u
...
def get_registrable_domain(hostname: str) -> str:
    ...
    The public suffix is the LONGEST matching multi-label suffix at the tail
    of the host (falling back to the last label for plain TLDs); the
    registrable domain is that suffix plus one label to its left.
    """
    if not hostname:
        return ""
    ...
    if len(parts) <= 2:
        return host
    # Longest multi-label suffix match wins (e.g. 'a.b.co.uk' must resolve
    # via 'co.uk', not the naive last-two-labels fallback).
    for length in _SUFFIX_LENGTHS:
        if length >= len(parts):
            continue
        if ".".join(parts[-length:]) in MULTI_LABEL_SUFFIXES:
            return ".".join(parts[-length - 1:])
    # fall back to last two labels
    return ".".join(parts[-2:])


def _aton_ipv4(h: str) -> tuple[bool, bool, str] | None:
    """
    Parse inet_aton-style IPv4 literals that ``ipaddress`` rejects: shortened
    forms ('127.1' -> 127.0.0.1, '10.1' -> 10.0.0.1) and hex/octal parts
    ('0x7f.1'). These resolve to loopback/private addresses on many OS
    resolvers, so treating them as hostnames would be an SSRF bypass.
    Returns None when the host is not an IPv4-ish literal at all.
    """
    if not re.fullmatch(r"(?:0[xX][0-9a-fA-F]+|\d+)(?:\.(?:0[xX][0-9a-fA-F]+|\d+)){1,3}", h):
        return None
    ...
    if len(nums) == 4:
        ...
        # inet_aton: all but the last part are single bytes; the last part
        # fills all REMAINING bytes ('127.1' -> 127.0.0.1, '127.0.1' ->
        # 127.0.0.1).
        if any(n > 255 for n in nums[:-1]):
            return None
        last_bits = 8 * (4 - (len(nums) - 1))
        if last_bits < 8 or nums[-1] >= (1 << last_bits):
            return None
        vals = list(nums[:-1])
        for shift in range(last_bits - 8, -1, -8):
            vals.append((nums[-1] >> shift) & 0xFF)

    ip = ipaddress.ip_address(".".join(map(str, vals)))
...
def parse_ip(host: str) -> tuple[bool, bool, str | None]:
    ...
    try:
        ...
        # inet_aton-style short forms ('127.1') must be treated as IPs,
        # never as hostnames — SSRF hardening.
        aton = _aton_ipv4(h)
        if aton is not None:
            return aton
...
def parse_url(raw_url: str, strict: bool = False) -> ParsedURL:
    ...
    if port is not None:
        p.port = port
        # An explicitly written scheme default (:443/:80) is still the default
        p.default_port = (p.scheme == "https" and port == 443) or (p.scheme == "http" and port == 80)
    elif p.scheme == "https":
        p.port = 443
        p.default_port = True
    elif p.scheme == "http":
    ...
    if not is_ip:
        ...
        # Free-host platforms whose domain is itself a multi-label public
        # suffix (github.io, blogspot.com, ...) host customer sites one label
        # further left, so also match the parent of the registrable domain.
        rd_parent = p.registrable_domain.split(".", 1)[-1]
        p.is_free_host = (
            p.registrable_domain in FREE_HOSTING
            or p.hostname in FREE_HOSTING
            or (p.registrable_domain.count(".") > 1 and rd_parent in FREE_HOSTING)
        )
